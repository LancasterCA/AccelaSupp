...
//		var id3 = pPINNum.substr(5,5);
		var pContactNum = pPINNum;
		var lLICNum = "" + AInfo["License Number"];
		var refNum = null;
...
//		myMessage += "<BR>" + "the PublicUserID:"+publicUserID;
		...
		var relateToCapId = aa.cap.getCapID(lLICNum).getOutput();
...
//		myMessage += "<BR>" + addObjPropertiesToMsg(capRelateContactArray);
		...
		if (capRelateContactArray) {
			for (yy in capRelateContactArray) {
				...
				var thisCon = capRelateContactArray[yy].getCapContactModel();
				var thisRefContactNum = thisCon.getRefContactNumber();

//		myMessage += "<BR>" + addObjPropertiesToMsg(thisCon);
//		myMessage += "<BR>" + "**-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- just finished printing thisCon -=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=- **";

				// if the refContactNum is blank, we must first create the refContact and use that seqNum to relate to the pUser.
				
				var nContactConSeqNum = thisCon.getContactSeqNumber();
				var nContactType = thisCon.getContactType();
				if ( nContactConSeqNum == pContactNum && nContactType == pContactType ) {
//					myMessage += "<BR>" + "*****************WINNER WINNER CHICKEN DINNER***********************";
//					myMessage += "<BR>" + "associating puser:"+ puserSeqA + " with thisRefContactNum:" + thisRefContactNum;

					var didItWork = aa.licenseScript.associateContactWithPublicUser(puserSeqA, thisRefContactNum);
//					if (didItWork.getSuccess()) myMessage += "<BR>it worked!" + addObjPropertiesToMsg(didItWork);
//					else myMessage += "<BR>" + "something didn't work..." + addObjPropertiesToMsg(didItWork);
					break;
				}
			}
		}
...
function sendDocUploadEmailToReviewer() {

    aa.print("function sendDocUploadEmailToReviewer - Begin");
    var recAltId = aa.cap.getCap(capId).getOutput().getCapID().getCustomID(); 
    var wfUserEmail = null;
	...
	var aWfHist = aa.workflow.getHistory(capId);
	if (aWfHist.getSuccess()) {
        var aTaskHist = aWfHist.getOutput(); 
    }
    else {
        aa.print("**ERROR: Failed to get workflow object: " + aWfHist.getErrorMessage());
        aa.print("function sendDocUploadEmailToReviewer - End");
        return false;
    }	
			
	for (i in aTaskHist) {
	  	aTask = aTaskHist[i]; 
	  	var aTaskStatus = aTask.getDisposition();
	  	aa.print("**Status: "+aTaskStatus);
				   	
		if(aTaskStatus=="Revisions Required"){  
			//get wf task updated by user
			var wfUpdateStaff = aTask.getTaskItem().getSysUser();
			var userObj = aa.person.getUser(wfUpdateStaff.firstName, "", wfUpdateStaff.lastName).getOutput();
			if (userObj){
				var wfUserEmail = userObj.getEmail();
				aa.print("**WF Staff Email: "+wfUserEmail);
				
				params = aa.util.newHashtable();
				reportFile = null;
				capIDString = recAltId;
				capName = null;
				capStatus = null;
				fileDate = null;
				balanceDue = null;
				...
				sendNotification("noreply@cityoflancasterca.org",wfUserEmail,"","PERMITS_DOC_UPLOAD_NOTIFICATION", params,reportFile);
			}	
...
function createRefContactsFromCapContactsAndLink_SLS(pCapId, contactTypeArray, ignoreAttributeArray, replaceCapContact, overwriteRefContact, refContactExists)
	{
...
//			var existingContact = refContactExists(p);  // Call the custom function to see if the REF contact exists
                         var existingContact = comparePeopleGeneric_SLS(p);  // Call the custom function to see if the REF contact exists
			...
			if (existingContact)  // we found a match with our custom function.  Use this one.
				...
				logDebug("Successfully created reference contact #" + refPeopleId);
				logDebug("Reference contact Org Name: " + p.getBusinessName());	
                                  logDebug("Reference contact First Name: " + p.getFirstName());			//Added to print contact info
                                  logDebug("Reference contact Last Name: " + p.getLastName());	
...
function comparePeopleGeneric_SLS(peop)
	{
	...
	//
	// current search method is by Business Name and Full Name only.  In order to use attributes enhancement 09ACC-05048 must be implemented
	//
	...
	if (peopResult.length > 0)
		...
		logDebug("Matches found! returning the first match : " + peopResult[0].getBusinessName() );
